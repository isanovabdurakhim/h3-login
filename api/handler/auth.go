...
func (h *Handler) Register(c *gin.Context) {
	...
	err := c.ShouldBindJSON(&register) // parse req body to given type struct
	if err != nil {
		h.handlerResponse(c, "register user", http.StatusBadRequest, err.Error())
		return
	}

	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(register.Password), 7)
	if err != nil {
	...
	id, err := h.storages.User().Create(context.Background(), &register)
	if err != nil {
		if err.Error() == `ERROR (SQLSTATE 23505)` {
	...
	user, err := h.storages.User().GetByID(context.Background(), &models.UserPKey{UserId: id})
	if err != nil {
...
func (h *Handler) Login(c *gin.Context) {
	...
	resp, err := h.storages.User().GetByID(context.Background(), &models.UserPKey{Login: login.Login})
	if err != nil {
